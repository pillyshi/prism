"""Unit tests for collection_evaluation and CollectionSynthesizer.sample_with_vectors."""
...
def _fit_synthesizer(n_features: int = 3, seed: int = 0) -> CollectionSynthesizer:
...
def test_evaluate_fit_std_diff_sign():
    ...
    X_orig = np.full((20, 1), 0.5)         # std ≈ 0
    X_sampled = rng.uniform(0, 1, (20, 1)) # std > 0
    result = evaluate_fit(X_orig, X_sampled)
...
# ---------------------------------------------------------------------------
# CollectionSynthesizer.sample_with_vectors
# ---------------------------------------------------------------------------
...
def test_sample_with_vectors_texts_match_sample():
    s = _fit_synthesizer()
    responses = ["text_a", "text_b", "text_c"]
    texts_a, _ = s.sample_with_vectors(3, llm=_make_llm(responses), rng=np.random.default_rng(42))
    texts_b = s.sample(3, llm=_make_llm(responses), rng=np.random.default_rng(42))
